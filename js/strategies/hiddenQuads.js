// js/strategies/hiddenQuads.js

export function solveHiddenQuads(board, candidates) {
    ...
    for (let type of units) {
        for (let i = 0; i < 9; i++) {
            ...
            for (let num = 1; num <= 9; num++) {
                if (mapNumToCells[num].length >= 2 && mapNumToCells[num].length <= 4) {
                ...
                for (let b = a + 1; b < potentialNumbers.length; b++) {
                    for (let c = b + 1; c < potentialNumbers.length; c++) {
                        for (let d = c + 1; d < potentialNumbers.length; d++) {
                            ...
                            if (unionCells.size === 4) {
                                const action = tryCleanNoise(candidates, [numA, numB, numC, numD], Array.from(unionCells), type, i);
...
function tryCleanNoise(allCandidates, keepValues, scopeCells, unitType, unitIndex) {
    ...
    if (modified) {
        ...
        return {
            ...
            reason: `Hidden Quad (${unitNames[unitType]}): Cuarteto oculto [${keepValues.join(',')}] en ${affectedCells.join(', ')}. Se eliminan los demás candidatos.`
        };
    }
    return null;
}
